...
echo "${BARMAN_EXPORTER_SCHEDULE} barman ps ax|grep [b]arman-exporter && /usr/local/bin/barman-exporter -l ${BARMAN_EXPORTER_LISTEN_ADDRESS}:${BARMAN_EXPORTER_LISTEN_PORT} -c ${BARMAN_EXPORTER_CACHE_TIME}" >> /etc/cron.d/barman
